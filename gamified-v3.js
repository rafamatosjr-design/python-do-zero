(() => {
  ...
  function injectNav(){
    ...
    const extra=[['trilha','🗺️','Trilha'],['praticar','🎯','Praticar'],['laboratorio','💻','Laboratório'],['desafios','🏆','Desafios']];
  ...
  function route(){const p=(location.hash.replace(/^#\/?/,'')||'inicio').split('/')[0]; injectNav(); if(p==='trilha'){renderTrail();return true} if(p==='praticar'){renderPractice();return true} if(p==='laboratorio'){renderLab();return true} if(p==='desafios'){renderChallenges();return true} return false}
