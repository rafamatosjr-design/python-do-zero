(() => {
  ...
  function renderTrail(){
    ...
    const nodes=groups.map((g,gi)=>{
      ...
      const missionHtml=Array.from({length:count},(_,k)=>{const id=from+k,isDone=done.has(id),isCurrent=id===current,isOpen=isDone||isCurrent||(id===1);return '<a class="mission-dot '+(isDone?'done ':isCurrent?'current ':'')+(isOpen?'':'locked')+'" '+(isOpen?'href="#/micro/'+id+'/0"':'aria-disabled="true"')+' title="Missão '+id+'">'+(isDone?'✓':id)+'</a>'}).join('');
      const nextHtml=active?'<div class="next-mission"><div><small>PRÓXIMA MISSÃO</small><strong>'+esc(microLessons[current-1].title)+'</strong><span>3–7 min · aprender fazendo</span></div><a class="btn primary" href="#/micro/'+current+'/0">Continuar →</a></div>':'';
      return '<section class="path-zone '+(unlocked?'':'locked')+'"><div class="path-zone-head"><span>'+icon+'</span><div><small>'+esc(label)+'</small><h2>'+esc(title)+'</h2><p>'+finished+'/'+count+' missões concluídas</p></div></div><div class="mission-line">'+missionHtml+'</div>'+nextHtml+'</section>';
    }).join('');
  ...
  function renderMicro(id,stepIndex=0){
    ...
    if(!body) body='<div class="feedback bad"><strong>Esta etapa não pôde ser exibida.</strong><p>Volte à trilha e abra a lição novamente. O erro foi impedido de deixar a tela em branco.</p><a class="btn primary" href="#/trilha">Voltar à trilha</a></div>';
  ...
  function route(){try{const parts=(location.hash.replace(/^#\/?/,'')||'inicio').split('/');const p=parts[0];injectNav();if(p==='trilha'){renderTrail();return true}if(p==='aprender'){renderLearn();return true}if(p==='micro'){renderMicro(Number(parts[1]),Number(parts[2])||0);return true}if(p==='praticar'){renderPractice();return true}if(p==='laboratorio'){renderLab();return true}if(p==='tutor'){renderTutor();return true}if(p==='desafios'){renderChallenges();return true}return false}catch(err){console.error('Falha ao renderizar rota gamificada',err);renderShell('<section class="card"><h1>Não foi possível abrir esta etapa</h1><p>O curso impediu uma tela em branco. Volte para a trilha e tente novamente.</p><a class="btn primary" href="#/trilha">Voltar para a trilha</a></section>');return false}}
